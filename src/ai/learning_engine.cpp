...
LearningEngine::LearningEngine()
    : tuning_state(TuningState::Idle), hardware_risk_alert(false),
      performanceNet(std::make_unique<NeuralNet>(2, 5, 1)) {}
...
void LearningEngine::addDataPoint(float intensity, int threads, float hashrate, float temp) {
    ...
    // Auto-train the Neural Network on the new data point
    // Normalize inputs: Intensity (0-32), Threads (0-32), Hashrate (0-2000)
    std::vector<double> inputs = { intensity / 32.0, threads / 32.0 };
    std::vector<double> targets = { hashrate / 2000.0 };
    performanceNet->train(inputs, targets, 0.1);
}

float LearningEngine::neuralPredictHashrate(float intensity, int threads) {
    std::lock_guard<std::mutex> lock(data_mutex);
    std::vector<double> inputs = { intensity / 32.0, threads / 32.0 };
    auto result = performanceNet->predict(inputs);
    return static_cast<float>(result[0] * 2000.0);
}
...
void LearningEngine::updateTuningProgress(float hashrate) {
    ...
    // Train the Neural Net during the Auto-Tune phase
    std::vector<double> inputs = { current_probe_intensity / 32.0, current_probe_threads / 32.0 };
    std::vector<double> targets = { hashrate / 2000.0 };
    performanceNet->train(inputs, targets, 0.2);
